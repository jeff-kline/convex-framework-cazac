...
than a proximal step. This mechanism --- KL descent for a Frank-Wolfe-type
greedy update, with a strongly convex smooth objective supplying the
sufficient-decrease and relative-error conditions --- is itself a known,
general-purpose template, independently instantiated by Akta\c{s} and
Kroer \cite{AK25} via a different (but interchangeable) citation to the
same family of abstract descent theorems; the CAZAC-specific content of
Theorem A is the reduction of CAZAC feasibility to $\max_{C_\tau}\|x\|^2$
(Prop.~\ref{prop:2}) that makes the template applicable here, not the
descent argument itself. Since neither that reduction nor the descent
argument uses CAZAC-specific structure beyond compactness, convexity, and
semialgebraicity, the argument transfers verbatim to a second example, the
elliptope $\El=\{X\succeq0: X_{ii}=1\}$ --- Felzenszwalb--Klivans--Paul's
...
not a numerical finding. \emph{Separately}, that generic trajectories
escape onto a rank-1 point specifically (Prop.~\ref{prop:ell4}):
\emph{Verified numerically only} ($n=5$, four trajectories) --- this is
not proved. Universal rank-1 convergence (Q7): \emph{Conjecture}.
