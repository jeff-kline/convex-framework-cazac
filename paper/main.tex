...
Bj\"orck's Legendre-symbol-phase construction, reduced to one unproved,
...
decays from $\approx1$ at small $n$ to $\approx0.3$ at $n=167$ (50 trials
per configuration; {\tt docs/main.tex}, Table~1, in the companion
repository \cite{CazacAlgorithmRepo}); whether
...
this is only a necessary condition for the full isotypic-block determinant
to be nonzero, not a sufficient one. Full regularity (R) at Bj\"orck points:
\textbf{open}, reduced to one unproved determinant-nonvanishing lemma
(specifically, a cross-term coincidence not addressed by the proved
sub-result above), \emph{verified numerically only} at 44 primes,
$p\in[5,200)$. Do not read this as ``proved'' --- the reduction is real
but incomplete.
...
{\tt verify\_bjorck\_crossterm\_gap.py}, {\tt verify\_popovic\_n4.py}) that
