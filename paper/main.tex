...
Zadoff--Chu tuple} --- we confirmed numerically that this family exists
...
of exact CAZAC points). \textbf{Bj\"orck--Saffari (1995)} \cite{BjorckSaffari95}:
...
sequences at every non-squarefree length unconditionally. Of the three,
...
finite up to phase --- which remains open in general (Haagerup settled
only the case $n$ prime); this is the natural generalization of exactly
...
A further reduction sharpens this open question and unifies it across
both residue classes: writing $\lambda_k$ for the order-$(p-1)$ character
with $\lambda_k(g_0)=e(k/(p-1))$ ($g_0$ a primitive root mod $p$) that
enters the isotypic decomposition, the block's determinant vanishes
exactly when $g(\lambda_k)=\pm g(\lambda_k\chi)$; the classical
Gauss--Jacobi identity $g(\psi)g(\chi)=J(\psi,\chi)\,g(\psi\chi)$ (for
nontrivial $\psi,\chi,\psi\chi$, verified here to machine precision)
re-expresses this as $J(\lambda_k,\chi)=\pm g(\chi)$ --- a single
condition independent of $p\bmod4$, where the prior reduction had only
been carried through for $p\equiv1\pmod4$, the residue class \emph{not}
containing $p=71$. Checked against all $44$ tested primes and every
nontrivial $k$ ($2045$ pairs total), no exact coincidence is found, and
$p=71,k=17$ remains the single closest approach in the now-unified range
(relative gap $\approx10^{-6}$, next-nearest $p=157$ at $\approx3\times10^{-3}$).
This is a genuine sharpening of the open question, not a resolution:
$J(\lambda_k,\chi)\ne\pm g(\chi)$ for every prime and every nontrivial $k$
remains open and unproved (script: {\tt experiments/verify\_bjorck\_jacobi\_unification.py}
in the companion repository \cite{CazacAlgorithmRepo}). A second, independent attempt at a clean closed-form nonvanishing argument
...
$c_N/n$, sampled ratios show no decay). This remaining gap is not a new
...
Corollary~\ref{cor:D2}) --- settled only at prime $n$, by Haagerup.
...
{\tt verify\_bjorck\_jacobi\_unification.py}) that
