...
constant-modulus time condition and a flat-spectrum frequency condition
simultaneously --- a nonconvex intersection of two tori. Relaxing the
equality constraints to convex inequalities gives a compact convex body
$C_\tau$ whose maximal-norm points, at $\tau=1$, are exactly the CAZAC
systems, and the natural algorithm iterates a second-order cone program
(SOCP) whose objective direction resets to the previous optimizer. The
central technical content is Theorem D: a regularity hypothesis (R) needed
for a sharp local convergence rate is pinned down exactly at every
Zadoff--Chu tuple and length $n$ (both parities; even $n$ was previously
open), via a mechanism new to this problem --- the DFT of a Zadoff--Chu
chirp is again a chirp, collapsing a rank computation to counting
solutions of $t^2\equiv0\bmod n$ --- so (R) holds iff $n$ is squarefree.
The mechanism is a reusable schema: a different closure fact (self-duality
of the Legendre symbol) extends it to a partial regularity result at
Bj\"orck's quadratic-phase construction, reduced to one unproved,
numerically-confirmed (44 primes) determinant lemma; the same specific
mechanism, by contrast, provably cannot reach cubic or higher-degree
phase (this does not rule out some other route). This underwrites Theorem
C (conditional on (R); unconditional when Theorem A's limit lies on a
Zadoff--Chu tuple's symmetry orbit and $n$ is squarefree): near a regular
CAZAC point the constraint slack grows \emph{linearly}, not quadratically
(KL exponent exactly $0$), and the iteration reaches the point exactly
after finitely many steps. The theory rests on Theorem A (unconditional):
every measurable selection converges to a single fixed point, via an
Attouch--Bolte--Svaiter Kurdyka--{\L}ojasiewicz descent argument applied
to a set-valued argmax update --- a known template, of which Akta\c{s} and
Kroer \cite{AK25} (2025, prior art) give a general instance for arbitrary
strongly convex objectives; our contribution is the reduction of CAZAC
feasibility to $\max_{C_\tau}\|x\|^2$ (Prop.~\ref{prop:2}) that makes the
template apply, not the descent argument itself. This reduction is
CAZAC-agnostic and transfers verbatim to a second example, the elliptope
$\El=\{X\succeq0: X_{ii}=1\}$ --- Felzenszwalb--Klivans--Paul's (2021) own
worked case, where their mechanism stalls at set-level convergence; we
prove single-point convergence there unconditionally. Their own
Theorem~20 further settles, by direct citation, that rank-1 points are
exactly the fixed points whose full neighborhood converges to them; a
$140$-trial sweep finds no exceptions, and whether the exceptional set
has measure zero remains open (Q7). Theorem B (unconditional,
obstruction): non-CAZAC fixed points exist and some are exposed, so no
deterministic selection rule can guarantee convergence to the CAZAC set
--- only a probabilistic statement over initializations is possible. We
close with the open-problems ledger (Q1 settled here as Theorem A;
Q2--Q7 open or partially resolved) and a tiered accounting of exactly
which claims are proved, proved conditionally, numerically verified, or
conjectural.
\end{abstract}
...
The hypothesis $x^0\in C_\tau$ costs nothing: the paper's own presolve
schedule (Section~1) hands the main phase a point in $C_{2.1}\setminus
C_1$, not in $C_1$, but every $x^{\ell+1}$ for $\ell\ge0$ is by
construction an argmax over $C_\tau$ and hence lies in $C_\tau$
automatically --- so re-indexing the main phase to start from its own
\emph{first} iterate (which is in $C_\tau$ regardless of where the
presolve output sat) restores the hypothesis with no loss; every result
below that depends on Prop.~\ref{prop:3} inherits this same one-step
re-indexing.
...
Definition of Section~1. The theorem then gives finite length and
...
For $n>3$, the fixed-point set $\mathcal F=\{X\in\El:X\in\arg\max_{Y\in\El}\langle
X,Y\rangle\}$ of the elliptope iteration is infinite, hence (being
semialgebraic) a genuine positive-dimensional continuum, not a singleton
or a finite set. (This restriction is load-bearing, not cosmetic: at
...
{\tt experiments/verify\_elliptope\_rank\_sweep.py}) of $140$ random and
...
$$\langle x^*,y\rangle_\R=(1-t)\bigl[\re y_0(0)+\re y_1(0)\bigr]+t\bigl[\re F(y_0)(0)+\re F(y_1)(0)\bigr]
\le(1-t)\cdot2+t\sqrt2\sqrt{S_y(0)}\le2(1-t)+2\sqrt2\,t=2(1+t^2),$$
using the amplitude bound directly on the first bracket and
Cauchy--Schwarz ($\re F(y_0)(0)+\re F(y_1)(0)\le\sqrt2\sqrt{S_y(0)}$) only
on the coupled second bracket, matching $\|x^*\|^2=2(1+t^2)$ for the
doubled point. Part (c) follows immediately from the existence of one such
retained point.
\end{proof}
...
kernel $e$, not (a subtlety a compressed first pass can miss) mere reality
...
the definition of the reality-defect kernel $e$ is the one place a
compressed first pass is liable to drop the conjugate term and
mis-simplify the zero set (an error the schema shares structurally with
the naive, incorrect transfer attempt at Bj\"orck's construction below,
Lemma~\ref{lem:bjaffine}'s own point of departure). The \emph{conclusion}
...
\begin{remark}[Prior art bearing on the lower bound: Popovi\'c's
generalized chirp-like sequences]
When $n=sm^2$ for positive integers $s,m$ (the largest valid $m$ is
$d_{\max}$ itself), Popovi\'c's 1992 construction \cite{Popovic92} gives an
explicit, elementary route to \emph{part} of Theorem~D: modulating a
Zadoff--Chu sequence of length $n$ by an \emph{arbitrary} unimodular
sequence of period $m$ (an $m$-real-parameter family, one free phase per
period) again satisfies the ideal periodic autocorrelation property ---
Popovi\'c's own Theorem~1, proved by an entirely different, purely
combinatorial argument with no DFT-closure or Gauss-sum machinery. At
$m=d_{\max}$ this gives an explicit $d_{\max}$-real-parameter family of
CAZAC points through every Zadoff--Chu tuple; we confirmed numerically
that this family exists exactly as claimed and has the predicted
dimension at $n=4$ ($d_{\max}=2$: an arbitrary period-$2$ phase pair gives
a verified $2$-parameter family of exact CAZAC points). This is real,
previously uncited prior art bearing on the \emph{existence} half of
Theorem~D --- that $\dim K$ can be as large as $d_{\max}$, not just $1$
--- via a route substantially different from, and more elementary than,
this paper's own. We have \textbf{not} worked out the precise
identification between Popovi\'c's explicit tangent family and this
paper's dependency space $K$ (a natural next step; the dimension match at
$n=4$ is evidence for, not proof of, such an identification), so this
disclosure is conservative: what Theorem~D adds beyond \cite{Popovic92} is
the \emph{exact} count (no more than $d_{\max}$, not merely at least
$d_{\max}$), the reusable chirp-DFT-closure mechanism itself --- which
extends, unlike Popovi\'c's construction, to Bj\"orck's non-Zadoff--Chu
sequences and to ruling out cubic-or-higher phase --- and the connection
to hypothesis (R) and Theorem~C's convergence rate.
\end{remark}
...
Theorem~D's closure property, for the same reason completing the square
is special to degree $2$: the addition law
...
Separately, quadratic Gauss sums have an exact closed-form evaluation
(magnitude $\sqrt n$ always); Weyl sums of degree $\ge3$ have no such exact
evaluation, only upper bounds, so even if some other closure map existed
it would not obviously carry the exact-modulus property Step~1 needs.
...
with $C=1-A$, where
$$p\equiv1\!\!\pmod4:\ A=\cos\alpha,\ B=i\sin\alpha,\ \alpha=\arccos\tfrac1{1+\sqrt p};
\qquad p\equiv3\!\!\pmod4:\ A=\tfrac{1+e^{i\beta}}2,\ B=\tfrac{1-e^{i\beta}}2=C,\ \beta=\arccos\tfrac{1-p}{1+p}.$$
...
Use $\alpha_0:=\alpha$ and $\beta_0:=\beta$, the same angles fixed in
Lemma~\ref{lem:bjaffine} (not a phase of $Bg$: for $p\equiv1\pmod4$,
$Bg=i\sin\alpha\cdot\sqrt p$ is purely imaginary, never real, so no
polar-decomposition phase of $Bg$ itself is what is meant here). The three
equations reduce, after eliminating the $m=0$ equation (which involves
only $\alpha,\gamma$ and is solved by $\alpha$ free), to:
$$p\equiv1\!\!\pmod4:\quad s\sqrt p\,\beta=0,\qquad -s\gamma\bigl(1+c(p-1)\bigr)=0\quad(c=\cos\alpha_0=\tfrac1{1+\sqrt p},\,s=\sin\alpha_0);$$
...
In the first case, $\alpha_0=\arccos\frac1{1+\sqrt p}\in(0,\pi/2)$ since
...
$\gamma=0$. In the second case, $\beta_0=\arccos\frac{1-p}{1+p}$ with
$\frac{1-p}{1+p}\in(-1,1)$ strictly for every $p\ge3$, so
$\beta_0\in(0,\pi)$ and $\sin\beta_0>0$; subtracting the two equations
gives $\gamma\bigl(\tfrac{p\sin\beta_0}2+\tfrac{\sin\beta_0}2\bigr)=0$,
i.e.\ $\gamma\sin\beta_0(p+1)/2=0$, and since $p+1\ne0$ and
$\sin\beta_0\ne0$, $\gamma=0$; then $\beta=0$ from either equation, since
the coefficient $\sqrt p(1-\cos\beta_0)/2$ is nonzero (as $\beta_0\ne0$).
...
$p=71$ the relevant gap is a factor of $\approx4\times10^3$ smaller than
at the next-nearest tested prime, a genuine near-degeneracy; the same
script now also runs an independent $60$-digit-precision (mpmath)
recomputation of this specific block, confirming the near-degeneracy is
not a floating-point artifact: the top singular value matches to
$\sim10^{-15}$ relative precision and the near-zero singular value moves
by only $\sim1\%$ of its own scale between float64 and $60$-digit
precision --- not by orders of magnitude, as a genuine cancellation
artifact would. Script: {\tt experiments/verify\_bjorck\_isotypic.py} in the
companion repository \cite{CazacAlgorithmRepo} (the $44$-prime
determinant check; a separately-run set of $14$ primes, all contained in
this same tested range, is used for the affine-closure check of
Lemma~\ref{lem:bjaffine}, in {\tt experiments/verify\_bjorck\_regularity.py}).
\textbf{The overall regularity
...
(remark after Corollary~\ref{cor:D2}): Popovi\'c's 1992 construction
\cite{Popovic92} gives, by an unrelated elementary method, an explicit
family matching the \emph{existence} half of this count ($\dim K$ can
reach $d_{\max}$) at $n=sm^2$; Theorem~D's own contribution is the exact
count and the reusable mechanism, not the existence of degeneracy per se.
...
{\tt verify\_bjorck\_regularity.py}, {\tt verify\_bjorck\_isotypic.py}) that
