...
($g_0$ a primitive root mod $p$). \textbf{One necessary condition for
$\det=0$ is now proved impossible, unconditionally}: $\rho_R(k)=0$ or
$\rho_N(k)=0$ requires $g(\lambda_k)=\pm g(\lambda_k\chi)$, and the
...
repository \cite{CazacAlgorithmRepo}). \textbf{This does not, however,
resolve $\det\ne0$}: $\det$ is a \emph{difference} of two terms, and
$u(k)v(k)-w(k)p_f(k)$ is a separate, nonzero quantity (an explicit closed
form, $-16\,a\,\epsilon\,g\,\gamma\sin^2\alpha$ for Bj\"orck's own
constants $a,\gamma,g$, itself never zero), so $\det=0$ remains logically
possible via $e(-k/q)\rho_R(k)^2u(k)v(k)=\rho_N(k)^2p_f(k)w(k)$ even
though every individual factor is nonzero --- and this cross-term
coincidence is checked directly, not assumed away: at the $k$ minimizing
$|\det|/p^2$ for each tested prime $p\equiv1\pmod4$ in $[5,60)$,
$\min(|\rho_R(k)|,|\rho_N(k)|)/\sqrt p$ stays in $[0.11,0.6]$ ---
\textbf{not small} --- even as $|\det|/p^2$ itself shrinks by three
orders of magnitude, showing the near-degeneracy traces to this
cross-term, not to $\rho_R$ or $\rho_N$ (script: {\tt
experiments/verify\_bjorck\_crossterm\_gap.py}, same repository). The
cross-term coincidence remains open and unresolved. Separately, the
...
claim at Bj\"orck points is therefore still open, not proved}, despite
this genuine partial progress: what is established is a full proof
restricted to the symmetry-invariant subspace, an unconditional proof
that one necessary condition for the remaining determinant to vanish is
never met, and strong (but non-uniform) numerical evidence, not a proof,
for the complementary directions as a whole.
\end{remark}
...
{\tt verify\_bjorck\_crossterm\_gap.py}) that
