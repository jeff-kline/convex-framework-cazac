...
ruling out one route (both twisted periods vanishing at once) by which
the remaining determinant could fail, and strong (but non-uniform)
numerical evidence, not a proof, for the complementary directions as a
whole.
\end{remark}
...
once, not the actual (separate, still-open) cross-term condition. Full
regularity (R) at Bj\"orck points:
...
directory; every numerical claim above is backed either by a named
script in that repository's {\tt experiments/} directory or, where noted
explicitly (Q6), by direct citation to a table in an earlier paper draft
kept in that repository's {\tt docs/} directory. Every claim above carries
