...
central technical content is Theorem D: a regularity hypothesis (R),
needed for a sharp local convergence rate, is proved unconditionally at
every Zadoff--Chu tuple and length $n$ (both parities; even $n$ was
previously open), via a mechanism new to this problem --- the DFT of a
Zadoff--Chu chirp is again a chirp, collapsing a rank computation to
counting solutions of $t^2\equiv0\bmod n$, so (R) holds iff $n$ is
squarefree. The mechanism is a reusable schema: a different closure fact
(self-duality of the Legendre symbol) extends it to a partial regularity
result at Bj\"orck's quadratic-phase construction, reduced to one
unproved, numerically-confirmed (44 primes) determinant lemma; we prove
it cannot reach cubic or higher-degree phase. This underwrites Theorem C
(conditional on (R); unconditional at squarefree Zadoff--Chu lengths):
near a regular CAZAC point the constraint slack grows \emph{linearly},
not quadratically (KL exponent exactly $0$), and the iteration reaches
the point exactly after finitely many steps. The theory rests on Theorem
A (unconditional): every measurable selection converges to a single fixed
point, via an Attouch--Bolte--Svaiter Kurdyka--{\L}ojasiewicz descent
argument applied to a set-valued argmax update --- a known template also
independently instantiated by Akta\c{s} and Kroer \cite{AK25} for general
...
--- the correct target class of theorem is probabilistic over
initializations. We close with the remaining open problems (Q1--Q7) and a
tiered ledger disclosing exactly which claims are proved, proved
